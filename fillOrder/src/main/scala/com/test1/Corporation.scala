...
class Corporation extends Actor with ActorLogging {
  ...
  def buyTrading(targetcalling:Calling,operator:Int=>Boolean ) : Unit = {
    sellList.find(x => {
      ...
      case Some(matchcalling) => {
        ...
        context.system.log.info(s"remove in selllist : ${sellList}")
      }
      case None => {
        ...
        context.system.log.info(s"add in buylist ${buyList}")
      }
    }
  }
  ...
  def sellTrading(targetcalling:Calling,operator:Int=>Boolean ) : Unit = {
    buyList.find(x => {
      ...
      case Some(matchcalling) => {
        ...
        context.system.log.info(s"remove in buylist : ${buyList}")
      }
      case None => {
        ...
        context.system.log.info(s"add in selllist ${sellList}")
      }
    }
  }
  ...
  def receive:Receive={
    case Buy(calling) => {
      context.system.log.info(s"in ${calling.Corporname}")
    ...
    case Cancel(action:Buy) =>{
      buyList = buyList.diff(Seq(action.Calling))
      sender() ! Complete(action.Calling.CallingId,action.Calling.UserId,s"Success Buy cancel")
    }
    case Cancel(action:Sell) =>{
      sellList = sellList.diff(Seq(action.Calling))
      sender() ! Complete(action.Calling.CallingId,action.Calling.UserId,s"Success Sell cancel")
    }
  }
