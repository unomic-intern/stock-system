...
class WaitingClient extends Actor{
  ...
  def DropOrder(targetList: mutable.HashMap[Long,Calling],callingId:Long) = {
    targetList.find(_._1 == callingId) match{
      ...
      case None => log.info("Response.buy.error : Calling Id doesn't exist")
    }
  }
  ...
  def receive :Receive ={
    case buy:Buy => {
      ...
      log.info(s"buy calling /WaitingClient \n${BuyOrderList}")
    }
    case sell:Sell => {
      SellOrderList += sell.Calling.CallingId -> sell.Calling
      endpoint forward sell
      log.info(s"sell calling /WaitingClient \n${SellOrderList}")
    }
    case Cancel(action) => {endpoint forward Cancel(action);log.info("cancel /WaitingClient")}
    //응답 오는거 보고 삭제하자
    case Finish(Buy(calling)) => {
      log.info(s"finish buy calling /WaitingClients \n${calling}")
    ...
    case Finish(Sell(calling)) => {
      log.info(s"finish sell calling /WaitingClients \n${calling}")
